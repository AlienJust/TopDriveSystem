...
{
    public class ReadEngineSettingsCommand : IRrModbusCommandWithReply, IRrModbusCommandResultGetter<IEngineSettings>,
        IRrModbusCommandWithTestReply
    {
        public IEngineSettings GetResult(byte[] reply)
        {
            ...
            {
                Pnom = new BytesQuad(reply[0], reply[1], reply[2], reply[3]).LowFirstUnsignedValue / 1000.0m,
                ...
                CosFi = new BytesPair(reply[16], reply[17]).LowFirstUnsignedValue / 100.0m,
                Eff = new BytesPair(reply[18], reply[19]).LowFirstUnsignedValue / 10.0m,
