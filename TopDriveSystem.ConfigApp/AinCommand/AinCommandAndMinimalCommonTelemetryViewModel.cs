...
namespace TopDriveSystem.ConfigApp.AinCommand
{
    internal class AinCommandAndMinimalCommonTelemetryViewModel : ViewModelBase
    {
        ...
        public bool IsSendingEnabled =>
            _sendingEnabledControl.IsSendingEnabled && _ainSettingsStorage.GetSettings(0) != null;

        /// <summary>
        ///     Частота в об/мин, перед отправкой преобразуется электрическую с точностью до дГц (0.1 Гц), 0.1 Гц = 0.3 об/мин
        ///     (когда две пары полюсов)
        /// </summary>
        public decimal? Fset
        {
            get => _fset;
        ...
        public decimal? FsetHz
        {
            get
            {
                ...
                if (ain1Settings != null && _fset.HasValue)
                    return
                        (int) (_fset * ain1Settings.Np / 0.6m) /
                        100.0m; // т.к. могу задавать частоту с точностью 1 сГц (0.01 Гц) - происходит округление до ближайшего минимального значения кратного 0.01
                return null;
            }
            set
            {
                var ain1Settings = _ainSettingsStorage.GetSettings(0);
                if (ain1Settings != null && value != null && ain1Settings.Np != 0)
                {
        ...
        public double? FsetReceived
        {
            get
            {
                ...
                var ain1Settings = _ainSettingsStorage.GetSettings(0);
                if (ain1Settings != null && ain1Settings.Np != 0)
                    return _telemetry.Fset.HighFirstSignedValue * 0.6 /
                           ain1Settings.Np; // полученная из телеметрии электрическая частота указана в 0.01Гц
                return null;
            }
        }

        /// <summary>
        ///     Минимальное изменение числа оборотов, которое соответсвует изменению частоты на 0.01Гц, однако если оно меньше 1.0,
        ///     то будет возвращена единичка
        /// </summary>
        ...
        public double? FsetSmallChange
        {
            ...
            {
                var ain1Settings = _ainSettingsStorage.GetSettings(0);
                if (ain1Settings != null && ain1Settings.Np != 0) return 0.6 / ain1Settings.Np;
        ...
        public short? MMinMaxAbs
        {
            get
            {
                if (_mmin < 0 && _mmax > 0 && _mmin + _mmax == 0 || _mmin == 0 && _mmax == 0) return _mmax;
        ...
        public RelayCommand SendAinCommandBits { get; }
        ...
        private void SendingEnabledControlOnSendingEnabledChanged()
        {
            _userInterfaceRoot.Notifier.Notify(() =>
            {
                ...
                SendAinCommandBits.RaiseCanExecuteChanged();
        ...
        private void SendCmdWithCommandMode(ushort commandMode)
        {
            try
            {
                ...
                _commandSenderHost.Sender.SendCommandAsync(_targerAddressHost.TargetAddress, cmd,
                    TimeSpan.FromSeconds(0.2), 2, (exception, bytes) => _userInterfaceRoot.Notifier.Notify(() =>
                    {
                        try
                        {
                            if (exception != null)
