...
{
    internal class ParamLoggerRegistrationPointThreadSafe : IParamLoggerRegistrationPoint, IParameterLogger
    {
        ...
        public void LogAnalogueParameter(string parameterName, double? value)
        {
            lock (_syncLoggers)
            {
                foreach (var logger in _registredLoggers) logger.LogAnalogueParameter(parameterName, value);
            }
        }
        ...
        public void LogDiscreteParameter(string parameterName, bool? value)
        {
            lock (_syncLoggers)
            {
                foreach (var logger in _registredLoggers) logger.LogDiscreteParameter(parameterName, value);
            }
        }
        ...
        public void RemoveSeries(string parameterName)
        {
            lock (_syncLoggers)
            {
                foreach (var logger in _registredLoggers) logger.RemoveSeries(parameterName);
            }
        }
