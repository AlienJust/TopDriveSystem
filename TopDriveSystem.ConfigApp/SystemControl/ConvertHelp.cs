...
{
    internal static class ConvertHelp
    {
        public static short ToInt16(byte lowByte, byte highByte)
        {
            if (BitConverter.IsLittleEndian) return BitConverter.ToInt16(new[] {highByte, lowByte}, 0);
            return BitConverter.ToInt16(new[] {lowByte, highByte}, 0);
        }

        public static ushort ToUInt16(byte lowByte, byte highByte)
        {
            if (BitConverter.IsLittleEndian) return BitConverter.ToUInt16(new[] {highByte, lowByte}, 0);
        ...
        public static string GetParamText(IList<byte> bytes, int zeroBasedRow, int oneBasedCol)
        {
            try
            {
                ...
                var firstCurrentRowByteIndex = zeroBasedRow * bytesInRow;

                var lowByteIndex = firstCurrentRowByteIndex + (oneBasedCol - 1) * bytesCountPerValue;
                var highByteIndex = firstCurrentRowByteIndex + (oneBasedCol - 1) * bytesCountPerValue + 1;
