    ...
    internal class TableViewModel : ViewModelBase
    {
        ...
        public void Update(IEngineTestResult testResult, IAinSettings settings, decimal f0)
        {
            ...
            if (settings != null)
            {
                if (settings.KiId == 0m)
                    _logger.Log(
                        "Параметр KiId из настроек АИН равен 0, вычисление IdIqTi не будет осуществлено из-за деления на 0");
                else if (f0 == 0m) _logger.Log("f0 равна 0, IdIqTi  не будет осуществлено из-за деления на 0");
                else IdIqTi = 1.0m / (settings.KiId * f0);
            }
            else
            {
                _logger.Log("Настройки АИН неизвестны (null), вычисление IdIqTi не будет осуществлено");
            }
            ...
            if (settings != null)
            {
                if (settings.KiFi == 0m)
                    _logger.Log(
                        "Параметр KiFi из настроек АИН равен 0, вычисление FluxTi не будет осуществлено из-за деления на 0");
                else if (f0 == 0m) _logger.Log("f0 is zero, FluxTi  не будет осуществлено из-за деления на 0");
                else FluxTi = 1.0m / (settings.KiFi * f0);
            }
            else
            {
                _logger.Log("Настройки АИН неизвестны (null), вычисление FluxTi не будет осуществлено");
            }
            ...
            if (settings != null)
            {
                if (settings.KiW == 0m)
                    _logger.Log(
                        "Параметр KiW из настроек АИН равен 0, вычисление SpeedTi не будет осуществлено из-за деления на 0");
                else if (f0 == 0m) _logger.Log("f0 is zero, SpeedTi  не будет осуществлено из-за деления на 0");
                else SpeedTi = 1.0m / (settings.KiW * f0);
            }
            else
            {
                _logger.Log("Настройки АИН неизвестны (null), вычисление SpeedTi не будет осуществлено");
            }
