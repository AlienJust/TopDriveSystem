...
{
    internal class Group04ParametersViewModel : ViewModelBase
    {
        ...
        public Group04ParametersViewModel(ICommandSenderHost commandSenderHost, ITargetAddressHost targerAddressHost,
            IUserInterfaceRoot uiRoot, ILogger logger, INotifySendingEnabled sendingEnabledNotifier)
        {
            ...
            ReadCycleCmd =
                new RelayCommand(ReadFunc, () => _sendingEnabledNotifier.IsSendingEnabled); // TODO: check port opened
        ...
        private void ReadFunc()
        {
            ...
            try
            {
                _commandSenderHost.Sender.SendCommandAsync(_targerAddressHost.TargetAddress, cmd,
                    TimeSpan.FromSeconds(0.1), 2, (exception, bytes) =>
                    {
            ...
            {
                _logger.Log(
                    "Не удалось отправить команду чтения телеметрии (версии ПО), убедитесь, что COM-порт открыт");
            }
        }
