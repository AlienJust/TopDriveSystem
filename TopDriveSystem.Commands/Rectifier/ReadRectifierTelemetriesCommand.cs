...
namespace TopDriveSystem.Commands.Rectifier
{
        ...
        IRrModbusCommandResultGetter<IList<IRectifierTelemetry>>, IRrModbusCommandWithTestReply
    {
        ...
        public IList<IRectifierTelemetry> GetResult(byte[] reply)
        {
            ...
            for (var i = 0; i < RectifiersCount; ++i)
                result.Add(
                    new RectifierTelemetrySimple(
                        ...
                        (short) (reply[i * BytesPerSingleRecifier + 10] +
                                 (reply[i * BytesPerSingleRecifier + 11] << 8)),
                        (short) (reply[i * BytesPerSingleRecifier + 12] +
                                 (reply[i * BytesPerSingleRecifier + 13] << 8)),
