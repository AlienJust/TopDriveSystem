...
{
    public class EngineTestLaunchCommand : IRrModbusCommandWithReply, IRrModbusCommandResultGetter<bool>,
        IRrModbusCommandWithTestReply
    {
        ...
        private byte[] GetBytesLe(int value)
        {
        ...
        private byte[] GetBytesLe(float value)
        {
            var bytes = BitConverter.GetBytes(value);
            if (BitConverter.IsLittleEndian) return bytes;
