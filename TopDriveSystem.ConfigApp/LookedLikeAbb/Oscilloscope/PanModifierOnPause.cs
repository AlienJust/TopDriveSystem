	...
	public class PanModifierOnPause : ChartModifierBase
    {
        ...
        private PanningState CreateInitialPanningState(ModifierMouseArgs e)
        {
            foreach (var xAxis in XAxes)
                if (HitTestableContainsMousePoint(xAxis, e)) // Dragging started on an x-axis, only pan that x-axis.
            ...
            foreach (var yAxis in YAxes)
                if (HitTestableContainsMousePoint(yAxis, e)) // Dragging started on an y-axis, only pan that y-axis.
                    return new PanningState
                    {
                        LastPoint = e.MousePoint,
        ...
        {
            var bounds = hitTestable.GetBoundsRelativeTo(RootGrid);
        ...
        {
            // Get region of axis
            var xAxisBounds = XAxis.GetBoundsRelativeTo(RootGrid);
            var yAxisBounds = YAxis.GetBoundsRelativeTo(RootGrid);
