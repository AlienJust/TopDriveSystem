...
{
    public class RtuModbusReadHoldingRegistersCommand : IRrModbusCommandWithReply,
        IRrModbusCommandResultGetter<IList<BytesPair>>, IRrModbusCommandWithTestReply
    {
        ...
        public IList<BytesPair> GetResult(byte[] reply)
        {
            ...
            var registers = new List<BytesPair>();
            for (var i = 0; i < _registersCountToRead; ++i)
        ...
        public string Name => "Read holding registers Modbus RTU command, first address: " + _firstRegisterAddress +
                              ", count: " + _registersCountToRead;
