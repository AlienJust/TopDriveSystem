...
{
    internal class BsEthernetSettingsViewModel : ViewModelBase
    {
        ...
        public ICommand WriteSettingCommand => _writeSettingsCommand;
        ...
        public IEnumerable<FtRoleViewModel> FtRoles => _ftRoles;
        ...
        private void ImportSettings()
        {
            ...
            var dialogResult = _windowSystem.ShowOpenFileDialog("Выберите файл с настройками БС-Ethernet",
        ...
        private void ExportSettings()
        {
            ...
            var dialogResult = _windowSystem.ShowSaveFileDialog("Выберите файл для сохранения настроек БС-Ethernet",
                "XML files|*.xml|All files|*.*");
            if (!string.IsNullOrEmpty(dialogResult))
                try
                {
                    var exporter = new BsEthernetSettingsXmlWorker(dialogResult);
                    var mac = new PhysicalAddress(MacAddress.Split('.')
                    ...
                    exporter.ExportSettings(new BsEthernetSettingsSimple(mac, ip, mask, gate, dns, _modbusAddress,
                        _driveNumber, _addressCan, ftRole));
        ...
        private void WriteSettings()
        {
            try
            {
                ...
                var mac = new PhysicalAddress(MacAddress.Split('.').Select(s => byte.Parse(s, NumberStyles.HexNumber))
                    .ToArray());
                var ip = new IPAddress(IpAddress.Split('.').Select(byte.Parse).ToArray());
                var mask = new IPAddress(Mask.Split('.').Select(byte.Parse).ToArray());
                var gate = new IPAddress(Gateway.Split('.').Select(byte.Parse).ToArray());
                ...
                var cmd = new WriteBsEthernetSettingsCommand(new BsEthernetSettingsSimple(mac, ip, mask, gate, dns,
                    ModbusAddress, DriveNumber, AddressCan, ftRole));
                    ...
                    , (exception, bytes) => _userInterfaceRoot.Notifier.Notify(() =>
                    {
                        try
                        {
                            ...
                            try
                            {
                                ...
                                mac = result.MacAddress.GetAddressBytes().Aggregate(mac,
